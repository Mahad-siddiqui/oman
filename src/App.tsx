...
import { Room, Booking, db } from './lib/localStorage';
import { loginAdmin, getStoredAdmin, storeAdmin, clearAdmin } from './lib/auth';
import { calculateTotalPrice } from './lib/utils';

type AppView = 'home' | 'rooms' | 'booking' | 'confirmation' | 'admin' | 'about' | 'contact' | 'services' | 'gallery' | 'mybookings';
...
function App() {
  ...
  const handleSearch = async (checkIn: string, checkOut: string, adults: number, children: number) => {
    ...
    try {
      const allRooms = await db.rooms.getAvailable();
      const existingBookings = await db.bookings.getAll();
      const activeBookings = existingBookings.filter(
        booking => booking.booking_status === 'Pending' || booking.booking_status === 'Confirmed'
      );
